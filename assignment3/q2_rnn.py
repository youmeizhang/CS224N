...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        add = max_length - len(sentence)
        
        if add > 0:
            new_sentence = sentence + ([zero_vector] * add)
            new_labels = labels + ([zero_label] * add)
            new_mask = [True] * len(sentence) + [False] * add
        else:
            new_sentence = sentence[:max_length]
            new_labels = labels[:max_length]
            new_mask = [True] * max_length
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits= mask_logits, 
                                                                             labels=mask_labels))
        ### END YOUR CODE
        return loss
