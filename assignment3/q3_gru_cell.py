...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            func = tf.contrib.layers.xavier_initializer(uniform=True)
            
            U_r = tf.get_variable("U_r", shape = [self.state_size, self.state_size], initializer = func, dtype = tf.float32)
            W_r = tf.get_variable("W_r", shape = [self.input_size, self.state_size], initializer = func, dtype = tf.float32)
            b_r = tf.get_variable("b_r", shape = [self.state_size, ], initializer = func, dtype = tf.float32)
            U_z = tf.get_variable("U_z", shape = [self.state_size, self.state_size], initializer = func, dtype = tf.float32)
            W_z = tf.get_variable("W_z", shape = [self.input_size, self.state_size], initializer = func, dtype = tf.float32)
            b_z = tf.get_variable("b_z", shape = [self.state_size, ], initializer = func, dtype = tf.float32)
            
            U_o = tf.get_variable("U_o", shape = [self.state_size, self.state_size], initializer = func, dtype = tf.float32)
            W_o = tf.get_variable("W_o", shape = [self.input_size, self.state_size], initializer = func, dtype = tf.float32)  
            b_o = tf.get_variable("b_o", shape = [self.state_size, ], initializer = func, dtype = tf.float32)
            
            z_t = 1 / (1 + tf.exp(-1 * (tf.matmul(inputs, W_z) + tf.matmul(state, U_z) + b_z)))
            r_t = 1 / (1 + tf.exp(-1 * (tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r)))
            o_t = tf.math.tanh(tf.matmul(inputs, W_o) + tf.matmul(r_t * state, U_o) + b_o)
