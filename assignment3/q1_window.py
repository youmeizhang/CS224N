    ...
    for sentence, labels in data:
        ...
        for i in range(len(sentence)):
            if i - window_size < 0 and i + window_size >= len(sentence):
                window_list.append([start] * (window_size - i))
                window_list.append(sentence)
                window_list.append([end] * (window_size + i - len(sentence) + 1)) 
                
            elif i - window_size < 0:
                window_list.append([start] * (window_size - i))
                window_list.append(sentence[0 : i + window_size + 1])
                
                
            elif i + window_size >= len(sentence):
                window_list.append(sentence[i - window_size : i + window_size])
                window_list.append([end] * (window_size + i - len(sentence) + 1))       
                
            else:
                window_list.append(sentence[i - window_size : i + window_size + 1])
      
            tmp = [y for sublist in window_list for x in sublist for y in x]
            
            windowed_data.append((tmp, labels[l]))
            window_list = []
            l += 1
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        #W = initializer(shape = [Config().n_window_features * Config().embed_size, Config().hidden_size ])
        #U = initializer(shape = [Config().hidden_size, Config().n_classes])
        W = tf.get_variable("w", shape = [Config().n_window_features * Config().embed_size, Config().hidden_size ], initializer = tf.contrib.layers.xavier_initializer(seed=1))
        U = tf.get_variable("u", shape = [Config().hidden_size, Config().n_classes], initializer = tf.contrib.layers.xavier_initializer(seed=2))
        
        
        #b1 = tf.Variable(tf.zeros([Config().hidden_size, ]))
        #b2 = tf.Variable(tf.zeros([Config().n_classes, ])) 
        b1 = tf.get_variable("b1", shape = [Config().hidden_size, ], initializer = tf.contrib.layers.xavier_initializer(seed=3))
        b2 = tf.get_variable("b2", shape = [Config().n_classes, ], initializer = tf.contrib.layers.xavier_initializer(seed=4))
