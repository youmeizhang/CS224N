...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        Args:
            inputs: is the input vector of size [None, ]
        ...
        with tf.variable_scope(scope):
            ...
            W_x = tf.get_variable("W_x", shape = [self.input_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer(seed=1))
            W_h = tf.get_variable("W_h", shape = [self.state_size, self.state_size], initializer = tf.contrib.layers.xavier_initializer(seed=2))
            b = tf.get_variable("b", shape = [self.state_size, ], initializer = tf.contrib.layers.xavier_initializer(seed=3))
            new_state = 1 / (1 + tf.exp(-1 * (tf.matmul(inputs, W_x) + tf.matmul(state, W_h) + b)))
            pass
            ### END YOUR CODE ###
