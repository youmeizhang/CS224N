...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    for k in range(K):
        samp = indices[k+1]
        #the latter part
        z = sigmoid(outputVectors[samp].dot(predicted))
        cost -= np.log(1.0 - z)
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    v_h = inputVectors[currentword_idx]
    
    for j in contextWords:
        u_idx = tokens[j]
        c_cost, c_gradIn, c_gradOut = word2vecCostAndGradient(v_h, u_idx, outputVectors, dataset)
    
        cost += c_cost
        gradIn[currentword_idx] += c_gradIn
        gradOut += c_gradOut
