...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 24 17:10:53 2019
...
def softmax(x):
    ...
    ### YOUR CODE HERE
    #tf.exp, tf.reduce_max, tf.reduce_sum, tf.expand_dims
    #z = x - tf.reduce_max(x)
    #out = tf.exp(z) / tf.reduce_sum(tf.exp(z))
    
    z = x - tf.reduce_max(x, axis=1)[:, tf.newaxis]
    out = tf.exp(z) / tf.reduce_sum(tf.exp(z), axis=1)[:, tf.newaxis]
