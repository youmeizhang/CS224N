...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 24 18:04:36 2019
...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("softmax") as scope:
            W = tf.Variable(tf.truncated_normal(shape = [Config().n_features, Config().n_classes], stddev = 0.1))
            b = tf.Variable(tf.zeros([Config().batch_size, 1]))
        
            yhat = tf.matmul(self.input_placeholder, W)
            yhat = yhat + b
